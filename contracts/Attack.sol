...
contract Attack{
    ...
    // Attack the `Game` contract by guessing the exact number because `blockhash` and `block.timestamp` is accessible publicly
    function attack() public {
        // `abi.encodePacked` takes in two params - `blockhash` and `block.timestamp`
        ...
        // keccak256 is a hashing function which takes in a bytes array and converts it into bytes32
